// setup-ipst-p6-2556-examset.js
// สร้าง ExamSetMetadata + ตั้ง questionNo/score สำหรับข้อสอบสสวท ป.6 2556
// รัน: node server/prisma/setup-ipst-p6-2556-examset.js

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const SOURCE = 'ipst-p6-2556';
...
async function main() {
  ...
  if (!existing) {
    await prisma.examSetMetadata.create({
      data: {
        grade: 'p6',
        year: '2556',
        label: 'สสวท คณิตศาสตร์ ป.6 ปี 2556 (สอบคัดเลือกรอบที่ 1)',
        timeLimitSec: 7200,
        blueprint: [
  ...
  if (questions.length === 0) {
    console.log('⚠️  ยังไม่มีโจทย์ — รัน seed-questions-ipst-p6-2556.js ก่อน');
    return;
  }
