// setup-ipst-p6-2554-examset.js
// สร้าง ExamSetMetadata + ตั้ง questionNo/score สำหรับข้อสอบสสวท ป.6 2554
// รัน: node server/prisma/setup-ipst-p6-2554-examset.js

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const SOURCE = 'ipst-p6-2554';
...
async function main() {
  ...
  if (!existing) {
    await prisma.examSetMetadata.create({
      data: {
        grade: 'p6',
        year: '2554',
        label: 'สสวท คณิตศาสตร์ ป.6 ปี 2554 (ฉบับที่ 1)',
        timeLimitSec: 7200, // 2 ชั่วโมง (120 นาที)
        blueprint: [
          { section: 1, count: 40, score: 5 }, // 40 ข้อ ข้อละ 5 คะแนน = 200 คะแนน
        ],
        isOfficial: true,
  ...
  // 2) ดึงโจทย์ทั้งหมดและกรองเฉพาะ source=ipst-p6-2554
  ...
  if (questions.length === 0) {
    console.log('⚠️  ยังไม่มีโจทย์ — รัน seed-questions-ipst-p6-2554.js ก่อน');
    return;
  }
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const qNo = String(i + 1).padStart(3, '0'); // '001' … '040'
    const score = 5;
